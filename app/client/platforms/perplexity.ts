...
export class PerplexityApi implements LLMApi {
  ...
  path(path: string): string {
    ...
    if (
      ...
      !baseUrl.startsWith(ApiPath.Perplexity)
    ) {
  ...
  async chat(options: ChatOptions) {
    ...
    const requestPayload: PerplexityRequestPayload = {
      messages,
      stream: options.config.stream,
      model: modelConfig.model,
      temperature: modelConfig.temperature,
      presence_penalty: modelConfig.presence_penalty,
      frequency_penalty: Math.max(modelConfig.frequency_penalty, 1), // ensure frequency_penalty is at least 1
      top_p: modelConfig.top_p,
      return_citations: true,
    };

    console.log("[Request] Perplexity payload: ", requestPayload);
    ...
    try {
      const chatPath = this.path(Perplexity.ChatPath);
      const chatPayload = {
        method: "POST",
      ...
      if (shouldStream) {
        ...
        fetchEventSource(chatPath, {
          ...
          async onopen(res) {
            ...
            console.log(
              "[Perplexity] request response content type: ",
              contentType,
            );
          ...
          onmessage(msg) {
            ...
            try {
              ...
              console.log("[Perplexity] response:", json);
              ...
              console.error("[Perplexity] parse error", text);
        ...
        console.log("[Perplexity] response:", res);
